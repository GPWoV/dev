...
Stage4::Stage4() : total_virus(15)
{
	...
	*/
	...
	stage4_music_ = Mix_LoadMUS("../../Resources/stage4.mp3");
	if (!stage4_music_)
	{
		printf("Couldn't load the Intro music  %s\n", Mix_GetError());
	}

	Mix_FadeInMusic(stage4_music_, -1, 2000);
...
void Stage4::Update()
{
	...
	if ((virus_delay > 330) && (respawn_count < total_virus)) {
		virus_delay = 0;
		respawn_count++;
		virus_list.push_back(new Virus({ 1200 + rand() % 20 * 60,rand() % 31 * 10 + 120,3,50,300,round,30,true }));
	}
