...
Stage2::Stage2() : total_virus(12)
{
	...
	*/
	...
	stage2_music_ = Mix_LoadMUS("../../Resources/stage2.mp3");
	if (!stage2_music_)
	{
		printf("Couldn't load the Intro music  %s\n", Mix_GetError());
	}

	Mix_FadeInMusic(stage2_music_, -1, 2000);
...
void Stage2::Update()
{
	...
	if ((virus_delay > 165) && (respawn_count < total_virus)) {
		virus_delay = 0;
		respawn_count++;
		virus_list.push_back(new Virus({ 1200, rand() % 36 * 10 + 120,3,50,200,round,10,true }));
	}
